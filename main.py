...
os.system('cp models/mv.py runs2/') # bkp of train procedure
os.system('cp models/mv_utils.py runs2/')
os.system('cp pointnet2_pyt/pointnet2/utils/pointnet2_modules.py runs2/')
os.system('cp pointnet2_pyt/pointnet2/utils/pointnet2_utils.py runs2/')
# os.system('cp utils/csrc/ellipsoid_query.c runs2/')
# os.system('cp cfgs/config_ssn_cls.yaml runs2/')
...
def check_inp_fmt(task, data_batch, dataset_name):
    if task in ['cls', 'cls_trans']:
        ...
        assert b1 == b2
        assert y == 3
        # print(DATASET_NUM_CLASS[dataset_name])
...
def save_checkpoint(id, epoch, model, optimizer,  lr_sched, bnm_sched, test_perf, cfg):
    model.cpu()
    path = f"./runs2/{cfg.EXP.EXP_ID}/model_{id}.pth"
    torch.save({
        'cfg': vars(cfg),
        'epoch': epoch,
...
def load_best_checkpoint(model, cfg):
    path = f"./runs2/{cfg.EXP.EXP_ID}/model_best.pth"
...
def entry_train(cfg, resume=False, model_path=""):
    ...
    log_dir = f"./runs2/{cfg.EXP.EXP_ID}"
...
def rscnn_vote_evaluation(cfg, model_path, log_file):
    ...
    assert cfg.EXP.DATASET in ["scanobjectnn_dgcnn"]
    loader_test = create_dataloader(split='test', cfg=cfg)
...
if __name__ == '__main__':
    ...
    os.system('cp $cmd_args.exp_config runs2/')
